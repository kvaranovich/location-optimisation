...
function parse_commandline()
  ...
  @add_arg_table! s begin
    ...
    "--q"
      ...
      required = true
    "--r"
      help = """radius of coverage in meters/seconds, e.g. [3000.0, 4500.0, 6000.0];"""
      arg_type = Float64
      required = true
    "--seed"
...
include("../optimisation/mip/mclp_model.jl")
...
const R = args["r"] #radius of coverage for each facility
...
const PATH_OUT = (
    ...
    * "_q"*string(Q) * "_R"*string(Int(round(R))) * "_seed"*string(SEED) * ".json"
)
Random.seed!(SEED)
